...
class BaseRunner:
    def __init__(self, settings, config):
        ...
        self.writer = SummaryWriter(log_dir=f"{self.settings.OUTPUT_PATH}/runs/{self.config.model_name}")
    ...
    def train(self, fold):
        ...
        with tqdm(range(1, self.settings.EPOCH)) as pbar:
            for epoch in pbar:
                ...
                # Log to TensorBoard
                self.writer.add_scalar(f"Fold{fold}/Train_Loss", trn_loss, epoch)
                self.writer.add_scalar(f"Fold{fold}/Valid_Loss", val_loss, epoch)
                self.writer.add_scalar(f"Fold{fold}/Valid_Score", score, epoch)
